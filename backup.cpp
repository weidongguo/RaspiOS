...
void HTTPClient::Request(const char* songName){
	...
	CString Header;
  //
  // Header.Format ("GET /search.php?query=adele HTTP/1.0\r\n"
...
\\
  ...
  //   CLogger::Get()->Write("HTTPClient", LogDebug, "%c", *(phase_2_link+i));
  // }
...
token = my_strstr(Buffer,"<div>");
    ...
    // printf("songlink.. %s\n", token1 );
    strcpy(final_link, token1);
    CLogger::Get()->Write("HTTPClient", LogDebug, "final_link..\n %s", final_link);

		len = strlen(token1);
    token=saveptr;
  }while(strcmp(token1+len-4,"mp3")!=0 && cnt<4); // If last 3 characters are mp3
	return songlink;
}
